...
Item {
    ...
    property Item qmlParent: (root.parent && root.parent.parent) ? root.parent.parent : null
    property int parentDragIndex: qmlParent?.dragIndex ?? -1
    property real parentDragDistance: qmlParent?.dragDistance ?? 0
    property real dragIndexDiff: Math.abs(parentDragIndex - (index !== undefined ? index : 0))
