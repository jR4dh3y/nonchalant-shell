...
- **Surface Ledger Exhaustion**: When auditing interaction or styling behavior across UI surfaces, map out every nested delegate, list item, icon trigger, and overlay. Never rely on shallow heuristic searches that only hit top-level controls and miss nested delegates.
- **Cohesive Delegate Scaling**: When applying tactile feedback (`scale: pressed ? 0.96 : 1.0`), ensure both background and foreground content scale together. Applying scale only to a background rectangle leaves sibling labels unscaled or unselected delegates completely dead.
- **State Machine Invariants**: Never bypass state transition methods (e.g. setting state variables directly rather than invoking transition methods that clean up grabs, cancel timers, and orchestrate animations).
- **Anti-Tampering Invariant**: Never add dummy comments or weaken tests to bypass failing test assertions. Update assertions cleanly to match architectural invariants.
- **Layout & Binding Loops**: In reactive UI layouts, avoid bidirectional or circular dependency cycles between parent dimensions and child implicit dimensions.
- **Asymmetric Morphing & Spring Dynamics**: When expanding elements (like Dynamic Island panels), use organic spring dynamics (`Easing.OutBack` with overshoot e.g. 1.25–1.28) for natural rebound. When collapsing or hiding elements, strictly use monotonic curves (`Easing.OutQuad` or `Easing.InCubic`) to prevent visual stretch/swelling on close.
